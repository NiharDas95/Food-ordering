...
function Listing(props) {
    ...
    //     updateData(updatedList);
    // }
    // const filteredData = masterData.filter(item => item.catagory === catag);
    return (
        <MyContext.Consumer>
            {({dishes=[], decrementCart, incrementCart}) => (
                <div className="cardswraper">
                    <h1>components Working with {catag}</h1>
                    {dishes.filter(item => item.catagory === catag).map((item, index) => {
                        return (
                            <Card>
                                <Itemcard desc={item.desc} key={item.key} price={item.price} index={index}
                                    cartQty={item.cartQuantity} incrementHandler={() => incrementCart(item.key-1)} decrementHandler={() => decrementCart(item.key-1)} />
                            </Card>
                        )
